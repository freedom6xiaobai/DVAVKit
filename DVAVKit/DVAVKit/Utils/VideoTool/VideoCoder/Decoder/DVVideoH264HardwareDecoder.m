...
#import "DVVideoH264HardwareDecoder.h"
#import "DVVideoError.h"
#import "DVVideoPacket.h"

@interface DVVideoH264HardwareDecoder () {
...
@implementation DVVideoH264HardwareDecoder

@synthesize delegate = _delegate;

- (instancetype)initWithSps:(NSData *)sps pps:(NSData *)pps delegate:(id<DVVideoDecoderDelegate>)delegate {
    self = [super init];
    if (self) {
        self.sps = sps;
        self.pps = pps;
...
- (void)initVideoSession {
    ...
    if (!self.sps || !self.pps) {
        NSLog(@"[DVVideoH264HardwareDecoder ERROR]: sps/pps为空, 初始化失败");
        return;
    }
    
    OSStatus status;
    CMVideoFormatDescriptionRef descriptionRef;
    const uint8_t *const paramSetPointers[2] = {self.sps.bytes, self.pps.bytes};
    const size_t paramSetSizes[2] = {(size_t)self.sps.length, (size_t)self.pps.length};
    ...
    VTDecompressionOutputCallbackRecord callBackRecord = {
        .decompressionOutputCallback = vtH264DecompressionOutputCallback,
...
- (void)printLog:(NSString *)message {
    NSLog(@"[DVVideoH264HardwareDecoder LOG]: %@",message);
}


#pragma mark - <-- Callback -->
void vtH264DecompressionOutputCallback(void * CM_NULLABLE decompressionOutputRefCon,
    ...
    if (status != noErr) {
        VideoCheckStatus(status, @"decompression to h264 error");
        return;
    }
    
    
    
    DVVideoH264HardwareDecoder *decoder = (__bridge DVVideoH264HardwareDecoder *)decompressionOutputRefCon;
    if (!decoder.delegate) {
        NSLog(@"[DVVideoH264HardwareDecoder ERROR]: delegate 为 nil");
        return;
    }
