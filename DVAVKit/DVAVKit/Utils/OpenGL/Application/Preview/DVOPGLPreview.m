...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        [self initCacheRef];
        [self initLocation];
        self.displayQueue = dispatch_queue_create("com.dv.avkit.opgl.preview", NULL);
    }
    return self;
}
...
- (void)setupVerticesIndeicsWithWidth:(int)width height:(int)height {
    ...
    CGRect sampleRect = AVMakeRectWithAspectRatioInsideRect(ratio, self.lastLayerBound);
...
- (void)displayWithPixelBuffer:(CVPixelBufferRef)pixelBuffer {
    ...
    dispatch_async(self.displayQueue, ^{
        [weakSelf _displayWithPixelBuffer:pixelBuffer];
