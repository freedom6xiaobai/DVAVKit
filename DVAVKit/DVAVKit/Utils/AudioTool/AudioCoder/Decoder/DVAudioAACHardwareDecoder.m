...
- (void)initAudioConverter {
    ...
    status = AudioConverterNewSpecific(&_inputBasicDesc,
                                       ...
                                       &_converterRef);
    if (status == noErr) {
        NSLog(@"[DVAudioEncoder LOG]: 创建硬解码器成功 -> %u",(unsigned int)_outputBasicDesc.mFormatID);
        return;
    } else {
    ...
    status = AudioConverterNewSpecific(&_inputBasicDesc,
                                       ...
                                       &_converterRef);
    if (status == noErr) {
        NSLog(@"[DVAudioEncoder LOG]: 创建软解码器成功 -> %u",(unsigned int)_outputBasicDesc.mFormatID);
        return;
    } else {
